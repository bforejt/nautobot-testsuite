...
SEMANTICS = {
    ...
    "iosxe_config": (
        ...
        "running-config and startup-config, so an expectation for it should use key "
        "'*-config'. Context: per config the line_count, the byte counts, "
        "volatile_lines_stripped and the lifted header facts (last_change_at/by; "
        "nvram_updated_at/by, the last save; no_change_since_restart), or present:false with "
    ...
    "iosxe_crash_files": (
        ...
        "Only files count: directories such as tracelogs/, whose dates move with "
        "routine logging, are never keyed or counted. Keys 'active|<file>' / "
        "'stby|<file>' for the active and standby members' "
    ...
    "iosxe_switch_stack": (
        ...
        "design). Context carries member and port totals. Not-present where the "
        "platform rejects the stacking commands (a C9500-16X does)."
    ),
